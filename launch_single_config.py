...
def single_time_experiment(env_name, n_pop, n_env, n_step, n_repetition):
    ...
    """
    env = brax_env.create(env_name=env_name, legacy_spring=True)
    rng = jax.random.PRNGKey(0)
    workers = {"mml": mapP_mapE_laxS(env), "fmm": forS_mapP_mapE(env)}
    # warmup:
...
def main():
    ...
    parser.add_argument(
        "--env_name", type=str, default="halfcheetah", help="brax environment name"
    )
    parser.add_argument(
        "--n_repetition",
        type=int,
        default=10,
        help="number of repetition for the experiment",
    )
    parser.add_argument("--n_pop", type=int, default=100, help="size of population")
    parser.add_argument(
        "--n_env", type=int, default=10, help="number of environments for each ind"
    )
    parser.add_argument(
        "--n_step", type=int, default=100, help="number step for each episodes"
    )
    args = parser.parse_args()
    print("args :", args)
    single_time_experiment(
